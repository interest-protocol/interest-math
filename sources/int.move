...
public fun div_down(a: Int, b: Int): Int {
    if (is_positive(a)) {
        ...
        if (is_positive(b)) {
            // B is positive
            return from_u256(u256::div_down(a.value, b.value))
            // Return positive
        } else {
            // B is negative
            return neg_from_u256(u256::div_down(a.value, abs(b).value))
            // Return negative
        }
    } else {
        // A is negative
        if (is_positive(b)) {
            // B is positive
            return neg_from_u256(u256::div_down(abs(a).value, b.value))
            // Return negative
        } else {
            // B is negative
            return from_u256(u256::div_down(abs(a).value, abs(b).value))
            // Return positive
        }
    }
}
...
public fun div_up(a: Int, b: Int): Int {
    if (is_positive(a)) {
        ...
        if (is_positive(b)) {
            // B is positive
            return from_u256(u256::div_up(a.value, b.value))
            // Return positive
        } else {
            // B is negative
            return neg_from_u256(u256::div_up(a.value, abs(b).value))
            // Return negative
        }
    } else {
        // A is negative
        if (is_positive(b)) {
            // B is positive
            return neg_from_u256(u256::div_up(abs(a).value, b.value))
            // Return negative
        } else {
            // B is negative
            return from_u256(u256::div_up(abs(a).value, abs(b).value))
            // Return positive
        }
    }
}
